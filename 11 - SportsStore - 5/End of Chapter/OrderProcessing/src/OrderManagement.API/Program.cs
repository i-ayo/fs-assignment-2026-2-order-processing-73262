...
// ── Bootstrap logger: captures startup errors before config is fully loaded ───
// Without this, any exception during DI setup (bad connection string, missing
// credential, etc.) produces NO console output — very hard to diagnose.
...
// ── appsettings.Local.json — gitignored file with real RabbitMQ password ─────
// Loaded after appsettings.json so it overrides the "Secrets" placeholder.
// For CI/CD the env var RabbitMQ__Password overrides this automatically.
...
// Scans this assembly for all IRequestHandler<,> and INotificationHandler<>
// implementations and registers them automatically.
builder.Services.AddMediatR(cfg =>
    cfg.RegisterServicesFromAssembly(typeof(Program).Assembly));

// ── AutoMapper ────────────────────────────────────────────────────────────────
// Registers IMapper using the OrderMappingProfile (Order→OrderResponse etc.).
...
{
    Log.Fatal(ex, "OrderManagement.API terminated unexpectedly during startup");
}
finally
{
    Log.CloseAndFlush();
